...
@PrepareForTest(VirtualMachine.State.class)
public class ConfigDriveNetworkElementTest {
    ...
    @InjectMocks @Spy private NetworkModel _networkModel = new NetworkModelImpl();
    ...
    public void testStart() {
        StateMachine2 mock = mock(StateMachine2.class);
        when(mock.registerListener(any(StateListener.class))).thenReturn(true);

        PowerMockito.mockStatic(VirtualMachine.State.class);
        BDDMockito.given(VirtualMachine.State.getStateMachine()).willReturn(mock);

        _configDrivesNetworkElement.start();

        verify(mock, times(1)).registerListener(any(StateListener.class));
    }
